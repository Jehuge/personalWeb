...
- `limit` 参数：统一设置为最大值 30
  - **首页接口**：10-20（首页展示，数据量小）
  - **所有列表接口**：30（统一限制，防止单次获取过多数据）
- `skip` 参数：必须 >= 0
- 防止通过单次请求获取所有数据，强制分页访问

**为什么设置 30？**
- 平衡用户体验和服务器负载
- 30 条数据通常足够单页展示（一般网页每页显示 10-20 条）
- 有效防止恶意爬虫一次性获取大量数据
- 结合速率限制（60次/分钟），即使每次请求30条，爬取全部数据也需要更多时间
- 降低数据库查询压力，提升响应速度
- 更严格的限制，增强反爬虫效果
