            ...
            <div className="hidden md:flex items-center space-x-2 flex-1 justify-center">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`relative z-10 transition-transform duration-300 ${isCurrent ? 'scale-105' : 'group-hover:scale-105'
                        }`}
                    >
                      {item.label}
                    </span>
                    {/* 柔和的背景光晕 */}
                    <span
                      className={`pointer-events-none absolute inset-0 bg-gradient-to-r from-gray-500/30 via-gray-400/15 to-gray-600/20 blur-xl transition-opacity duration-300 ${isCurrent ? 'opacity-100' : 'opacity-0 group-hover:opacity-80'
                        }`}
                    />
                    {/* 底部滑动条指示器 */}
                    <span
                      className={`pointer-events-none absolute left-4 right-4 -bottom-1 h-[2px] rounded-full bg-gradient-to-r from-gray-500 via-gray-600 to-gray-500 transform origin-center transition-transform duration-300 ${isCurrent ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
                        }`}
                    />
                  </Link>
                );
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`inline-block transition-transform duration-200 ${isCurrent ? 'scale-[1.03]' : 'group-hover:scale-[1.03]'
                        }`}
                    >
                      {item.label}
                    </span>
